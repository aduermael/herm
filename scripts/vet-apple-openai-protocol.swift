...
private struct CPSLOpenAIProtocolChecks {
    static func main() throws {
        ...
        try assertChatRequestExposesOnlyLocalSandboxTool()
    ...
    private static func assertChatRequestExposesOnlyLocalSandboxTool() throws {
        let request = CPSLOpenAIChatRequest(
            model: "test-model",
            messages: [.system("system"), .user("run")],
            tools: [.localSandboxExec],
            toolChoice: "auto",
            stream: true
        )
        ...
        guard let tools = object["tools"] as? [[String: Any]], tools.count == 1,
            ...
            throw CheckFailure("chat request did not expose exactly the local sandbox tool")
        }
    }
