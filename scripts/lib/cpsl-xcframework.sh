...
cpsl_xcframework_has_library_arch() {
	...
	[ -f "$info" ] || return 1
	awk -v want_platform="$want_platform" -v want_variant="$want_variant" -v want_arch="$want_arch" '
		/<dict>/ {
			depth++
			if (depth == 2) {
				platform = ""
...
cpsl_xcframework_library_identifier() {
	...
	[ -f "$info" ] || return 1
	awk -v want_platform="$want_platform" -v want_variant="$want_variant" '
		/<dict>/ {
			depth++
			if (depth == 2) {
				platform = ""
...
cpsl_xcframework_write_source_stamp_value() {
	...
	cpsl_xcframework_source_stamp_expected "$source_root" "$source_revision" >"$stamp_path"
}
...
cpsl_xcframework_write_source_stamp() {
	...
	cpsl_xcframework_write_source_stamp_value "$stamp_path" "$source_root" "$source_revision"
}
...
cpsl_xcframework_source_stamp_matches_value() {
	...
	expected=$(cpsl_xcframework_source_stamp_expected "$source_root" "$source_revision") || return 1
...
cpsl_xcframework_source_stamp_matches() {
	...
	cpsl_xcframework_source_stamp_matches_value "$stamp_path" "$source_root" "$source_revision"
}
