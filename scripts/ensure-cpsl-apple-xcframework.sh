...
Environment:
  ...
  IOS_DEVICE_TARGETS, IOS_SIMULATOR_TARGETS, MACOS_TARGETS
...
cpsl_ensure_rebuild_reason() {
	...
	if [ "$require_submodule_source_stamp" -eq 1 ]; then
		cpsl_xcframework_source_stamp_matches "$source_stamp_path" "$default_cpsl_root" || {
			printf '%s\n' "$xcframework_path was not built from external/cpsl at the current revision"
			return 0
		}
	fi
...
if [ "$skip_mode" -eq 1 ]; then
	...
	if [ "$require_submodule_source_stamp" -eq 1 ]; then
		cpsl_xcframework_source_stamp_matches "$source_stamp_path" "$default_cpsl_root" || \
			die "$xcframework_path was not built from external/cpsl at the current revision; rerun without --skip"
	fi
	cpsl_xcframework_satisfies_targets "$xcframework_info" "$ios_device_targets" "$ios_simulator_targets" "$macos_targets" || \
