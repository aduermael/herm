...
Use this as a compatibility interface for simple shell-style commands or when the user asks for shell syntax. Prefer the `luau` tool for native sandbox module work, structured data, and repeated automation. Bash-compatible input is transpiled into sandbox Luau; this is not host command execution. Do not assume apt, brew, pip, npm, Node, C/C++ compilers, system package installs, background services, daemons, or host shell access.

Do not use this tool to run Lua or Luau. Commands such as `lua`, `luau`, `lua -e`, and `luau -e` are not available in the sandbox shell; call the `luau` tool directly instead.
