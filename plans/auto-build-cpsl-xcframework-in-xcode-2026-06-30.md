...
- [ ] 1b: Add `scripts/ensure-cpsl-apple-xcframework.sh` implementing visionOS guard, `HERM_CPSL_REBUILD=1`, full-framework validation, staleness check, optional lock file, and delegation to `build-cpsl-apple-xcframework.sh` with `APPLE_PLATFORMS=ios macos`.
